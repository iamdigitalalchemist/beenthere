...
export async function POST(_request: Request, { params }: RouteProps) {
  ...
  // Only allow reopen within 48 hours of starts_at.
  const startsAt = new Date(event.startsAt);
  const reopenDeadline = new Date(startsAt.getTime() + 48 * 60 * 60 * 1000);
  if (new Date() > reopenDeadline) {
    return NextResponse.json(
      { error: "This event can no longer be reopened — the 48-hour window has passed." },
      { status: 403 },
    );
  }
  ...
  await pool.query(
    `update beenthere.events set status = 'active' where public_id = $1`,
    [publicId],
  );

  logger.info("event_reopened", { event_id: event.id, public_id: publicId });
